...
Built jar output:
- `build/libs/foxcraft-0.1.0-SNAPSHOT.jar`
...
- If you are not flying, FoxCraft tries to place you on safe ground and cancels the teleport if none is found.
...
- If a player enters a world without that permission, FoxCraft disables their flight automatically.
...
- Uses the same safe teleport rules as other FoxCraft teleports.
...
- The teleported player stays in the air if flying, otherwise FoxCraft requires safe ground.
...
- Translation files are stored in `plugins/foxcraft/translations/`.
...
- `storage.mysql.table-prefix` controls the table name prefix used by FoxCraft.
...
- Runtime translations are stored in `plugins/foxcraft/translations/`.
...
## Current limitations
- Teleport commands only support online players.
